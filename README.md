...
+ ContainerコンポーネントとPresentationalコンポーネントの分離<br>
...
+ 非同期通信を行う際のコードを謎に分離<br>
  ...
  axiosでAPIと非同期通信を行っているのですが、なぜか二つのディレクトリ(actionsとapis)にある別ファイルに跨って実装してしまっており、見づらいし記述量が無駄に増えるだけだったので途中から一つのフ   ァイルにまとめました。<br>
...
+ テスト
  ...
  * このアプリを開発し始める前に勉強のために作っていた簡単なCRUDのものにはRSpecやJestなどを導入して実装し、テストも学んでいたが今回取り入れてないので、今後必ず取り入れる。
  
+ 画像、動画編集機能

  * インスタのストーリーのように、選択した画像や動画をGLを使って簡単な編集(文字の追加)などできるようにしたい
...
+ 画像形式をWebPにする
  ...
  * 現在、画像を速く表示するために画像のクオリティーを下げることをしているが、Lamdaとか使ってWebP形式にしてみて、クオリティーを落とさずに画像を速く表示できるか試してみたい。
...
+ デプロイ
  ...
  * API側をデプロイする。以前AWSのEC2などを使い2回デプロイしたことがあるが、Lamdaを使ってみたい。
 
大量にあるのでほんの一部ですが、このようなことを今後取り入れていきたいと思っています。

*READMEを読んでいただきありがとうございました！*
